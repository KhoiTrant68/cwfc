...
def setup(dataset="cifar", H=32, N=1024, ae_steps=3000, seed=0, device=None,
    ...
    print(f"[setup] device={device} dataset={dataset} H={H} "
          f"AE recon mse={rec:.4f} quantized-recon PSNR={ae_psnr:.2f}dB "
          f"condition y_hat shape={y_shape}")
    return {"data": data, "ae": ae, "y_shape": y_shape, "device": device,
            "N": N, "ae_psnr": ae_psnr}
...
def run_frontier(ctx, etas, embeds=("raw", "proj8", "pool"), seeds=(0, 1, 2),
    ...
    if out:
        ...
        with open(out, "w") as f:
            json.dump({"ae_psnr": ctx["ae_psnr"], "y_shape": list(ctx["y_shape"]),
                       "loss": loss, "frontier": res}, f, indent=2)
        print(f"saved -> {out}")
    return res
...
if __name__ == "__main__":
    ...
    run_frontier(ctx, etas=[0, 0.3, 1, 3, 30], embeds=["pool"], seeds=[0],
                 steps=400, loss="debiased")
